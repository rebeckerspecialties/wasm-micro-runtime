...
mode=${1:-asan-ubsan}
build_dir=${2:-$(mktemp -d "${TMPDIR:-/tmp}/wamr-execution-quota-$mode.XXXXXX")}
...
compile_args=(
  ...
  -DWASM_ENABLE_COMPONENT_MODEL=0
  ...
  -I "$repo_root/core/iwasm/libraries/thread-mgr"
...
/usr/bin/clang "${compile_args[@]}" \
  "$test_dir/allocation_quota_execution_test.c" "${link_args[@]}" \
  -o "$build_dir/allocation-quota-execution-test"
"${runtime_environment[@]}" "$build_dir/allocation-quota-execution-test"
