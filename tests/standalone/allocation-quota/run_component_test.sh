...
mode=${1:-asan-ubsan}
build_dir=${2:-$(mktemp -d "${TMPDIR:-/tmp}/wamr-component-quota-$mode.XXXXXX")}
...
if [[ -z "$bison_executable" && -x /opt/homebrew/opt/bison/bin/bison ]]; then
...
compile_args=(
  ...
  -DWASM_MEM_ALLOC_WITH_USER_DATA=1
...
/usr/bin/clang "${compile_args[@]}" \
  "$test_dir/allocation_quota_component_load_test.c" "${link_args[@]}" \
  -o "$build_dir/allocation-quota-component-load-test"
"${runtime_environment[@]}" "$build_dir/allocation-quota-component-load-test" \
  "$repo_root/tests/apple-app-store-interpreter/static_host_component.wasm" \
  "$repo_root/tests/unit/component/wasm-apps/large_shared_host_aliases.wasm" \
  "$repo_root/tests/unit/component/wasm-apps/complex.wasm"
