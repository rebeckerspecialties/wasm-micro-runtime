                                ...
                                int32_t *ret, int *err)
{
    ...
    if (offset > INT64_MAX || lseek(fd, 0, SEEK_CUR) < 0) {
        *err = offset > INT64_MAX ? EOVERFLOW : errno;
        *ret = -1;
        return;
    }

    /* The StreamResourceType tracks its own logical position and all file
       stream I/O uses pread/pwrite.  The duplicate owns the file lifetime but
       never mutates the descriptor's shared kernel cursor. */
                                  ...
                                  wasi_output_stream_t *ret, int *err)
{
    ...
    // Duplicate only for independent lifetime; append behavior is tracked in
    // StreamResourceType and applied per write without changing shared flags.
    int new_fd = fcntl(fd, F_DUPFD_CLOEXEC, 0);
    if (new_fd < 0) {
        *err = errno;
        *ret = -1;
                     ...
                     bool *end_of_stream, int *err)
{
    ...
    *end_of_stream = false;
    ...
    /* wasm_runtime_malloc takes uint32_t, while pread returns ssize_t and
       accepts a signed file offset. Reject every lossy conversion before
       allocating or entering the kernel. */
    if (length > UINT32_MAX || length > (uint64_t)SSIZE_MAX
    ...
    uint8_t *buf = wasm_runtime_malloc((uint32_t)length);
    if (!buf) {
        *err = ENOMEM;
        return;
    }

    ssize_t s = pread(fd, buf, (size_t)length, (off_t)offset);
    if (s < 0) {
    ...
    if ((uint64_t)s < length) {
        *end_of_stream = true;
        if (s > 0) {
            uint8_t *new_buf = wasm_runtime_realloc(buf, (uint32_t)s);
            if (!new_buf) {
                wasm_runtime_free(buf);
                *err = ENOMEM;
                return;
                      ...
                      wasi_filesize_t *ret, int *err)
{
    ...
    *ret = 0;
    if (buf_len > (uint64_t)SSIZE_MAX || offset > (uint64_t)INT64_MAX) {
        *err = EOVERFLOW;
        return;
    }

    /* Serialize positional descriptor writes with logical file streams and
       truncation. In particular, an append stream must observe either the
       file state before this write or after it, never an interleaving between
       its EOF lookup and pwrite. */
    wasi_p2_file_io_lock();
    ssize_t s = pwrite(fd, buf, (size_t)buf_len, (off_t)offset);
    int saved_error = s < 0 ? errno : 0;
                               ...
                               wasi_directory_entry_stream_t *ret, int *err)
{
    ...
    /* dup()/F_DUPFD share one open-file description and therefore one kernel
       directory offset. Open "." relative to the capability instead, yielding
       a fresh OFD that cannot escape the supplied directory. This new_fd is
       deliberately the single ownership-transfer seam for a future FD lease:
       once opened, either fdopendir owns it or this function closes it. */
    int new_fd = openat(fd, ".", O_RDONLY | O_DIRECTORY | O_CLOEXEC);
    if (new_fd < 0) {
        *err = errno;
        *ret = NULL;
        return;
    ...
    if (!dir) {
        ...
        close(new_fd); // Clean up the fresh descriptor on failure.
        *err = saved_error;
        *ret = NULL;
        return;
    }
                             ...
                             WasiP2NativeFdQuotaLease *out_fd_lease, int *err)
{
    ...
    *err = wasi_beneath_path_ref_init(exec_env, fd, path, &path_ref);
    ...
    if (r < 0) {
        ...
        *err = saved_error == EXDEV ? EPERM : saved_error;
        *ret = -1;
    ...
    *ret = r;
    ...
    if (r < 0) {
        *err = wasi_sandbox_error(saved_error);
        *ret = -1;
        wasi_p2_native_fd_quota_release(&fd_lease);
        wasi_beneath_path_ref_destroy(&path_ref);
        return;
...
wasi_filesystem_set_size(wasi_descriptor_t fd, wasi_filesize_t size)
{
    ...
    /* Keep ftruncate atomic with respect to append-stream EOF selection and
       positional descriptor/stream writes. */
    wasi_p2_file_io_lock();
    int truncate_result = ftruncate(fd, (off_t)size);
    int saved_error = truncate_result < 0 ? errno : 0;
    wasi_p2_file_io_unlock();
